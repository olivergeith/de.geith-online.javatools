...
public class Utils {
	 ...
	 */
	/*
	 * Old method that ran very slowely and doesnt logical correct, how does
	 * dividing something at 10-4 by 10,000 convert it to 10 -3 public static
	 * GregorianCalendar getDateOf(final BigInteger fileTime) { final
	 * GregorianCalendar result = new GregorianCalendar(1601, 0, 1); // lose
	 * anything beyond milliseconds, because calendar can't handle // less value
	 * BigInteger time = fileTime.divide(new BigInteger("10000")); //$NON-NLS-1$
	 * final BigInteger maxInt = new BigInteger(String
	 * .valueOf(Integer.MAX_VALUE)); while (time.compareTo(maxInt) > 0) {
	 * result.add(Calendar.MILLISECOND, Integer.MAX_VALUE); time =
	 * time.subtract(maxInt); } result.add(Calendar.MILLISECOND,
	 * time.intValue()); return result; }
	 */

	/**
	 * Date values in ASF files are given in 100 ns (10 exp -4) steps since
	 * first
	 * 
	 * @param fileTime
	 *            Time in 100ns since 1 jan 1601
